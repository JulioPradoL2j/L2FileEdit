...
import net.sf.l2jdev.listeners.FormatListener;
import net.sf.l2jdev.util.DebugUtil;
import net.sf.l2jdev.util.Util;
import net.sf.l2jdev.xml.exceptions.CycleArgumentException;
...
public class DescriptorParser
{
					...
					for (Node defNode = defsNode.getFirstChild(); defNode != null; defNode = defNode.getNextSibling())
					{
						if (defNode.getNodeName().equals("enum"))
						{
							...
							else
							{
								...
								final Map<Integer, String> eTypes = _enumMap.computeIfAbsent(defName, _ -> new HashMap<>());
								final Map<String, String> eReverseTypes = _enumReverseMap.computeIfAbsent(defName, _ -> new HashMap<>());
	...
	private static boolean parseBoolNode(Node node, String name, boolean def)
	{
	...
	private static String parseStringNode(Node node, String name, String def)
	{
		if ((node.getAttributes() == null) || (node.getAttributes().getNamedItem(name) == null))
		{
			return def;
