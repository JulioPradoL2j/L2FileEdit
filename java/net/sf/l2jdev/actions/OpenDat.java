...
import net.sf.l2jdev.util.DebugUtil;
import net.sf.l2jdev.xml.CryptVersionParser;
import net.sf.l2jdev.xml.Descriptor;
import net.sf.l2jdev.xml.DescriptorParser;
import net.sf.l2jdev.xml.DescriptorReader;
...
public class OpenDat extends ActionTask
{
	...
	public OpenDat(L2FileEdit l2ClientDat, String structureChronicle, File file)
	{
		super(l2ClientDat);
	...
	public static ByteBuffer decrypt(File file, Collection<DatCrypter> decryptors, boolean mass) throws Exception
	{
		...
		if (!file.exists())
		{
			if (!mass)
			{
				L2FileEdit.addLogConsole("File " + fileName + " does not exist.", true);
			}
			return null;
		}
		...
		if (!file.canRead())
		{
			if (!mass)
			{
				L2FileEdit.addLogConsole("Unable to read " + fileName + " file.", true);
			}
			return null;
		}
		...
		if (fis.available() < 28)
		{
			if (!mass)
			{
				L2FileEdit.addLogConsole("The file " + fileName + " is too small.", true);
			}
			fis.close();
			return null;
		}
		...
		if (!header.matches("Lineage2Ver(\\d{3})"))
		{
			if (!mass)
			{
				L2FileEdit.addLogConsole("File " + fileName + " not encrypted. Skip decrypt.", true);
			}
			crypt = false;
		}
		...
		if (crypt)
		{
			...
			if (!mass)
			{
				L2FileEdit.addLogConsole("File " + fileName + " encrypted. " + header + " decrypt ...", true);
			}
			...
			if (crypter == null)
			{
				if (!mass)
				{
					L2FileEdit.addLogConsole("Error decrypt " + fileName + " file.", true);
				}
				return null;
			}
			...
			if (!mass)
			{
				L2FileEdit.addLogConsole("Decrypt " + fileName + " file successfully by " + crypter.getName() + " decrypter.", true);
			}
		}
		else
		{
			...
			catch (IOException exc)
			{
				if (!mass)
				{
					L2FileEdit.addLogConsole("Error reading" + fileName + "  file.", true);
				}
			}
		}
		return buffer;
	...
	public static String start(ActionTask actionTask, double weight, String structureChronicle, File file, boolean mass) throws Exception
	{
		...
		else
		{
			if ((crypter == null) || !crypter.isUseStructure())
			{
				if (!mass)
				{
					L2FileEdit.addLogConsole("Unknown file " + fileName + " type!", true);
				}
				return null;
			}
			...
			if (!mass)
			{
				L2FileEdit.addLogConsole("Read the file structure ...", true);
			}
			...
			if (text == null)
			{
				if (!mass)
				{
					L2FileEdit.addLogConsole("Structure is not found in the directory: " + structureChronicle + " file: " + fileName, true);
				}
				return null;
			}
		}
		...
		if (!mass)
		{
			L2FileEdit.addLogConsole("Completed.", true);
		}
