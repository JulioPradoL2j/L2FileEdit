...
public class SaveDat extends ActionTask
{
	...
	public SaveDat(L2FileEdit l2ClientDat, File file, String chronicle)
	{
		super(l2ClientDat);
		_file = file;
	...
	protected void action()
	{
		...
		if (_file.getName().endsWith(".dat") || _file.getName().endsWith(".txt"))
		{
			try
			{
				...
				{
					L2FileEdit.addLogConsole("Not found the structure of the file: " + _file.getName(), true);
				}
			}
			catch (Exception e)
			{
		...
		{
			L2FileEdit.addLogConsole("Unknown file " + _file.getName() + " type!", true);
			shouldContinue = false;
		}
		...
		if (shouldContinue && (buff != null))
		{
			...
			L2FileEdit.addLogConsole("Packed successfully by " + (crypter == null ? "crypter" : crypter.getName()) + " encrypter. Elapsed ".concat(String.valueOf(diffTime)).concat(" sec"), true);
			return;
		}
		
		L2FileEdit.addLogConsole("buff == null.", true);
	}
}
