...
import net.sf.l2jdev.clientcryptor.crypt.RSADatCrypter;
...
import net.sf.l2jdev.xml.DescriptorWriter;
...
public class MassTxtPacker extends ActionTask
{
	...
	public MassTxtPacker(L2FileEdit l2ClientDat, String chronicle, String path)
	{
		super(l2ClientDat);
	...
	{
		L2FileEdit.addLogConsole("Mass packer with using " + _chronicle + " chronicles by path [" + _path + "]", true);
		...
		{
			L2FileEdit.addLogConsole("Selected encryptor not RSA! Please select RSA encryptor.", true);
			return;
		}
		
		if (!encrypter.isEncrypt())
		{
			L2FileEdit.addLogConsole("Selected encryptor does not have encrypt RSA key.", true);
			return;
		}
		...
		{
			L2FileEdit.addLogConsole("Cannot create directory [" + packDir + "].", true);
			return;
		}
	...
	private static void pack(ActionTask actionTask, double weight, String chronicle, DatCrypter encrypter, File file, File packDir)
	{
		...
		L2FileEdit.addLogConsole("Start packing [" + file.getName() + "]...", true);
		...
		try
		{
			...
			if (file.getName().endsWith(".dat") || file.getName().endsWith(".txt"))
			{
				...
				{
					L2FileEdit.addLogConsole("Not found the structure of the file: " + file.getName(), true);
				}
			}
			...
			{
				L2FileEdit.addLogConsole("Unknown file [" + file.getName() + "] type!", true);
				shouldContinue = false;
			}
			...
			if (shouldContinue && (buff != null))
			{
				try
				{
					...
					L2FileEdit.addLogConsole("Success packed [" + file.getName() + "]", true);
				}
				catch (Exception e)
				{
