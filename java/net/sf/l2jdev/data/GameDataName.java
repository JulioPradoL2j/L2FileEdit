...
import net.sf.l2jdev.util.ByteReader;
import net.sf.l2jdev.util.ByteWriter;
import net.sf.l2jdev.util.Util;
import net.sf.l2jdev.xml.ParamNode;
...
public class GameDataName
{
		...
		try
		{
			...
			if (decCrypter.isEncrypt())
			{
				...
				if (file.exists())
				{
					...
					for (int i = 0; i < list.size(); ++i)
					{
						...
						{
							L2FileEdit.addLogConsole("GameDataName: Contains dublicate value: [" + name + "] in index[" + i + "]!", true);
						}
					}
					L2FileEdit.addLogConsole("GameDataName: Load " + _names.size() + " count.", true);
				}
				else
				{
					L2FileEdit.addLogConsole("GameDataName: File '" + file.getName() + "' not found!", true);
				}
			...
			else
			{
				...
				if (file.exists())
				{
					...
					if (fis.available() < 28)
					{
						L2FileEdit.addLogConsole(file.getName() + " The file is too small.", true);
						fis.close();
						return;
					}
					...
					{
						L2FileEdit.addLogConsole("GameDataName: File " + file.getName() + " not encrypted. Skip decrypt.", true);
						return;
					}
					
					if (Integer.parseInt(header.substring(11)) != decCrypter.getCode())
					{
						L2FileEdit.addLogConsole("GameDataName: File " + file.getName() + " encrypted code: " + header + ". Skip decrypt.", true);
						return;
					}
					
					L2FileEdit.addLogConsole("Unpacking [" + file.getName() + "]", true);
					...
					for (int size = ByteReader.readUInt(buff), j = 0; j < size; ++j)
					{
						...
						{
							L2FileEdit.addLogConsole("GameDataName: Contains dublicate value: [" + name2 + "] in index[" + j + "]!", true);
						}
					}
					L2FileEdit.addLogConsole("GameDataName: Load " + _names.size() + " count.", true);
				}
	...
	public String getString(File currentFile, DatCrypter crypter, int index, boolean mass) throws Exception
	{
		...
		try
		{
			...
			{
				L2FileEdit.addLogConsole("GameDataName: Not found string for index: " + index, true);
				return "[None]";
			}
			...
			if (!mass && val.isEmpty())
			{
				L2FileEdit.addLogConsole("GameDataName: String name Empty!!! Index: " + index + ", file: " + currentFile.getName(), true);
			}
	...
	public int getId(File currentFile, DatCrypter crypter, ParamNode node, String str, boolean mass) throws Exception
	{
		...
		{
			L2FileEdit.addLogConsole("GameDataName: String name not brackets!!! file: " + currentFile.getName() + " str: " + str + " node: " + node, true);
		}
		...
		if (str.isEmpty())
		{
			if (!mass)
			{
				L2FileEdit.addLogConsole("GameDataName: String name Empty!!! file: " + currentFile.getName() + ", node: " + node, true);
			}
			return -1;
		}
	...
	public void checkAndUpdate(String currentDir, DatCrypter crypter) throws Exception
	{
		...
		try
		{
			if (!_nameHash.isEmpty())
			{
				...
				L2FileEdit.addLogConsole("GameDataName: Packed " + _names.size() + " count.", true);
			}
		}
		finally
		{
