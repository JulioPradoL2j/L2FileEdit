...
public class DescriptorReader
{
	...
	public String parseData(ActionTask actionTask, double weight, File currentFile, DatCrypter crypter, Descriptor desc, ByteBuffer data, boolean mass) throws Exception
	{
		...
		if (desc.isRawData())
		{
			...
			if (!readVariables(currentFile, crypter, node, new HashMap<>(), data, builder, true, mass))
			{
				...
				L2FileEdit.addLogConsole(errorMsg, true);
			}
			else
			{
		...
		if (data.limit() > pos)
		{
			...
			L2FileEdit.addLogConsole("Unpacked not full " + data.position() + "/" + data.limit() + " diff: " + (data.limit() - pos), true);
		}
		return stringData;
	}
			...
			for (int j = 0; j < nodeSize; ++j)
			{
				...
				else
				{
					...
					else if (node.getEntityType().isVariable() && !readVariables(currentFile, crypter, node, vars, data, out, false, mass))
					{
						if (!mass)
						{
							L2FileEdit.addLogConsole(String.format("Error while parsing variable NAME[%s] TYPE[%s] in file NAME[%s]! Parsed data: %s", node.getName(), node.getType(), currentFile.getName(), out.toString()), true);
						}
	...
	private static boolean readVariables(File currentFile, DatCrypter crypter, ParamNode node, Map<String, Variant> vars, ByteBuffer data, StringBuilder out, boolean isRaw, boolean mass)
	{
		try
		{
			switch (node.getType())
